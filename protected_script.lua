...
function _G.HK_GetVal(id)
    ...
    if _G.HK_Settings_Raw and _G.HK_Settings_Raw[id] ~= nil then
        return _G.HK_Settings_Raw[id]
    end
    return HK_DefaultSettings[id] or 0
end
...
pcall(function()
    ...
    if f then
        ...
        if cached and cached ~= "" then
            ...
            -- HIỂN THỊ THÔNG BÁO CHÀO MỪNG LOBBY KHI ĐÃ ĐƯỢC VPS DUYỆT
            pcall(function()
                if active and not _G.DX_HasShownLobbyWelcome then
                    local GameStatus = _G.GameStatus or package.loaded["GameStatus"]
                    if GameStatus and GameStatus.IsInFightingStatus and not GameStatus.IsInFightingStatus() then
                        _G.DX_HasShownLobbyWelcome = true
                        local msgBox = package.loaded["client.slua.logic.common.logic_common_msg_box"] 
                                    or require("client.slua.logic.common.logic_common_msg_box")
                        if msgBox and msgBox.Show then
                            msgBox.Show(1, "[DX MOD]", "Đăng nhập thành công! Chào mừng bạn đến với Sảnh Chờ.", function() end, function() end, "Xác nhận", "Đóng")
                        end
                    end
                end
            end)
...
-- =========================== SERVER-DRIVEN CODE INJECTION ENGINE (ANTI-CRACK) ===========================
local DX_XOR_KEY = "DX_SECRET_KEY_2026_@#$"

local function DX_XorByte(a, b)
...
local function DX_DecryptPayload(hexStr)
    if not hexStr or #hexStr == 0 then return nil end
    local bytes = {}
    for i = 1, #hexStr, 2 do
    ...
    local keyBytes = { string.byte(DX_XOR_KEY, 1, #DX_XOR_KEY) }
    local res = {}
    for i = 1, #bytes do
        local k = keyBytes[((i - 1) % #keyBytes) + 1]
        table.insert(res, string.char(DX_XorByte(bytes[i], k)))
    end
    return table.concat(res)
end

function _G.DX_WipeCoreMemory()
    pcall(function()
        _G.DX_CoreLoaded = false
        _G.HK_Settings_Raw = {}
        _G.HK_FakeData = nil
        _G.HK_HWID_Hooked = nil
        _G.HK_DataOS_Hooked = nil
        _G.EnvRequiresUpdate = true
        _G.MagicUpdateVersion = (_G.MagicUpdateVersion or 1) + 1
    end)
end

function _G.DX_LoadCorePayload(payloadHex)
    pcall(function()
        if not payloadHex or #payloadHex == 0 then return end
        local luaCode = DX_DecryptPayload(payloadHex)
        if luaCode and #luaCode > 0 then
            local loaderFunc = load or loadstring
            if loaderFunc then
                local fn, err = loaderFunc(luaCode)
                if fn then
                    fn()
...
function _G.DX_CheckUIDWithAdminVPS(customMethod)
    local now = os.time()
    local interval = 30 -- Check mỗi 30 giây đối với Heartbeat/APPROVED
    ...
    local function onResponse(success, data)
        ...
        if success and data and #data > 0 then
            ...
            -- NẾU SERVER TRẢ VỀ DỮ LIỆU PAYLOAD MÃ HÓA TỪ VPS ➔ LOAD ĐỘNG VÀO RAM
            if active then
                local payloadHex = string.match(data, '"payload":"([^"]+)"')
                if payloadHex and #payloadHex > 0 then
                    _G.DX_LoadCorePayload(payloadHex)
                end
            else
                -- NẾU KHÔNG ACTIVE / BỊ BLOCK ➔ XÓA SẠCH MEMORY TÍNH NĂNG LÕI
                _G.DX_WipeCoreMemory()
            end
    ...
    if not SettingPageDefine.ModMenu or _G.LastVIPApprovalState ~= isApproved then
        ...
        for _, cat in ipairs(itemCategories) do
            table.insert(StackItemESP, {
                ...
                ExpandIndex = 0,
                GetFunc = function() return _G.HK_Settings_Raw[cat.key] == 1 end,
                SetFunc = function(_, value)
                    _G.HK_Settings_Raw[cat.key] = value and 1 or 0
                    _G.EnvRequiresUpdate = true
                    return true
                end
            })
            for _, wp in ipairs(cat.weapons) do
                table.insert(StackItemESP, {
                    ...
                    GetFunc = function() return _G.HK_Settings_Raw[wp.key] == 1 end,
                    SetFunc = function(_, value)
                        _G.HK_Settings_Raw[wp.key] = value and 1 or 0
                        _G.EnvRequiresUpdate = true
                        return true
                    end
                })
        ...
        local StackAimbotV2 = {
            ...
            -- AIMBOT SHOTGUN
            { Key = "ModMenu_AT_SG_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot Shotgun (Chỉ nhận Shotgun)", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSG == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSG = v and 1 or 0; _G.EnvRequiresUpdate = true; return true end },
            { Key = "ModMenu_AT_SG_AutoFire", UI = AliasMap.Switcher, Text = "      Tự Động Bắn lúc tự động bắn chịu khó bấm bắn nhận dame và auto bắn sẽ không lỗi dame", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGAutoFire == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGAutoFire = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_SG_IgKnock", UI = AliasMap.Switcher, Text = "      Bỏ Qua Địch Knock", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGIgKnock == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGIgKnock = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_SG_IgBot", UI = AliasMap.Switcher, Text = "      Bỏ Qua Bot", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGIgBot == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGIgBot = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_SG_Vis", UI = AliasMap.Switcher, Text = "      Check Tường (VisCheck)", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGVisCheck == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGVisCheck = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_SG_Prio", UI = AliasMap.Slider, Text = "      Ưu Tiên (1:Tâm 2:Gần 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGPrio or 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGPrio = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_SG_Bone", UI = AliasMap.Slider, Text = "      Vị Trí (1:Đầu 2:Ngực 3:Bụng 4:Hông)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGBone or 2 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGBone = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_SG_Cond", UI = AliasMap.Slider, Text = "      Điều Kiện (1:Bắn mới Aim, 2:Luôn Aim)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGCond or 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGCond = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_SG_Spd", UI = AliasMap.Slider, Text = "      Độ Mượt / Tốc Độ (1-100)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGSpeed or 80 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGSpeed = v return true end },
            { Key = "ModMenu_AT_SG_FOV", UI = AliasMap.Slider, Text = "      Vòng FOV (1-100)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGFOV or 40 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGFOV = v return true end },
            { Key = "ModMenu_AT_SG_Dist", UI = AliasMap.Slider, Text = "      Khoảng Cách (1-100m)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSGDist or 30 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSGDist = v return true end },
            
            -- SCOPE ALL (SÚNG THƯỜNG KHI MỞ SCOPE)
            { Key = "ModMenu_AT_ScopeAll_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot Mở Scope", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeAll == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeAll = v and 1 or 0; _G.EnvRequiresUpdate = true; return true end },
            { Key = "ModMenu_AT_ScopeAll_IgKnock", UI = AliasMap.Switcher, Text = "      Bỏ Qua Địch Knock", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeIgKnock == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeIgKnock = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_ScopeAll_IgBot", UI = AliasMap.Switcher, Text = "      Bỏ Qua Bot", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeIgBot == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeIgBot = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_ScopeAll_Vis", UI = AliasMap.Switcher, Text = "      Check Tường (VisCheck)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeVisCheck == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeVisCheck = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_ScopeAll_Prio", UI = AliasMap.Slider, Text = "      Ưu Tiên (1:Tâm 2:Gần 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopePrio or 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopePrio = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_ScopeAll_Bone", UI = AliasMap.Slider, Text = "      Vị Trí (1:Đầu 2:Ngực 3:Bụng 4:Hông)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeBone or 2 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeBone = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_ScopeAll_Cond", UI = AliasMap.Slider, Text = "      Điều Kiện (1:Bắn mới Aim, 2:Luôn Aim)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeCond or 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeCond = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_ScopeAll_Spd", UI = AliasMap.Slider, Text = "      Độ Mượt / Tốc Độ (1-100)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeSpeed or 40 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeSpeed = v return true end },
            { Key = "ModMenu_AT_ScopeAll_FOV", UI = AliasMap.Slider, Text = "      Vòng FOV (1-100)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeFOV or 20 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeFOV = v return true end },
            { Key = "ModMenu_AT_ScopeAll_Dist", UI = AliasMap.Slider, Text = "      Khoảng Cách (1-500m)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return math.floor((_G.HK_Settings_Raw.AimTouchScopeDist or 300) / 5) end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeDist = v * 5 return true end },
            { Key = "ModMenu_AT_ScopeAll_Pred", UI = AliasMap.Slider, Text = "      Dự Đoán Hướng Chạy", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopePred or 0 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopePred = v return true end },
            { Key = "ModMenu_AT_ScopeAll_Recoil", UI = AliasMap.Slider, Text = "      Bù Giật Tự Động Ghìm Tâm Khi Aim ( để tầm 3%-4% là ổn)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 0, MaxValue = 50, min = 0, max = 50, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeRecoil or 0 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeRecoil = v return true end },

            -- SCOPE SNIPER (SÚNG NGẮM/TỈA)
            { Key = "ModMenu_AT_Sniper_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot Mở Scope (Súng Ngắm/Tỉa)", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.HK_Settings_Raw.AimTouchScopeSniper == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchScopeSniper = v and 1 or 0; _G.EnvRequiresUpdate = true; return true end },
            { Key = "ModMenu_AT_Sniper_IgKnock", UI = AliasMap.Switcher, Text = "      Bỏ Qua Địch Knock", ExpandHandle = "ModMenu_AT_Sniper_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperIgKnock == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperIgKnock = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_Sniper_IgBot", UI = AliasMap.Switcher, Text = "      Bỏ Qua Bot", ExpandHandle = "ModMenu_AT_Sniper_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperIgBot == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperIgBot = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_Sniper_Vis", UI = AliasMap.Switcher, Text = "      Check Tường (VisCheck)", ExpandHandle = "ModMenu_AT_Sniper_Ex", GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperVisCheck == 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperVisCheck = v and 1 or 0 return true end },
            { Key = "ModMenu_AT_Sniper_Prio", UI = AliasMap.Slider, Text = "      Ưu Tiên (1:Tâm 2:Gần 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperPrio or 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperPrio = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_Sniper_Bone", UI = AliasMap.Slider, Text = "      Vị Trí (1:Đầu 2:Ngực 3:Bụng 4:Hông)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperBone or 1 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperBone = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_Sniper_Cond", UI = AliasMap.Slider, Text = "      Điều Kiện (1:Bắn mới Aim, 2:Luôn Aim)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperCond or 2 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperCond = math.floor(v+0.5) return true end },
            { Key = "ModMenu_AT_Sniper_Spd", UI = AliasMap.Slider, Text = "      Độ Mượt / Tốc Độ (1-100)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperSpeed or 30 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperSpeed = v return true end },
            { Key = "ModMenu_AT_Sniper_FOV", UI = AliasMap.Slider, Text = "      Vòng FOV (1-100)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperFOV or 20 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperFOV = v return true end },
            { Key = "ModMenu_AT_Sniper_Dist", UI = AliasMap.Slider, Text = "      Khoảng Cách (1-500m)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return math.floor((_G.HK_Settings_Raw.AimTouchSniperDist or 400) / 5) end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperDist = v * 5 return true end },
            { Key = "ModMenu_AT_Sniper_Pred", UI = AliasMap.Slider, Text = "      Dự Đoán Hướng Chạy (0-100)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return _G.HK_Settings_Raw.AimTouchSniperPred or 0 end, SetFunc = function(_, v) _G.HK_Settings_Raw.AimTouchSniperPred = v return true end }
        }
...
function BRPlayerCharacterBase:ReceiveBeginPlay()
    ...
    -- TỰ ĐỘNG CHẠY FAKE HWID + IP + FIREBASE + XID (PHẦN 24B) KHI VÀO TRẬN & HIỂN THỊ THÔNG BÁO (CHỈ VỚI LOCAL PLAYER)
    pcall(function()
        if _G.HK_RegenerateAllFakeData then _G.HK_RegenerateAllFakeData() end
        if _G.HK_InitializeHWIDHook then _G.HK_InitializeHWIDHook() end
        
        local isLocal = (self.Role == ENetRole.ROLE_AutonomousProxy)
        if not isLocal and self.IsLocallyControlled then
            isLocal = self:IsLocallyControlled()
        end
        
        if isLocal and _G.HK_NotifyFakeSuccess and not _G.HK_HasNotifiedThisMatch then
            _G.HK_HasNotifiedThisMatch = true
...
function BRPlayerCharacterBase:ReceiveEndPlay(EndPlayReason)
    pcall(function()
        ...
        if isLocal then
            _G.HK_HasNotifiedThisMatch = nil
        end
    end)
